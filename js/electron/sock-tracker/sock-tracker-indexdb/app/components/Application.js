...
class Application extends Component {
    constructor(props) {
        ...
        require("electron").ipcRenderer.on("delete-database", () => {
            this.deleteItems();
        });
    }
    componentDidMount() {
    ...
    deleteItem(item) {
        this.props.database.delete(item)
                           .then(this.fetchItems)
                           .catch(console.error);
    }

    deleteItems() {
        this.props.database.deleteAll()
                           .then(this.fetchItems)
                           .catch(console.error);
    }
    ...
    markAsPacked(item) {
        ...
        this.props.database.update(updatedItem)
                           .then(this.fetchItems)
                           .catch(console.error);
    }

    markAllAsUnpacked() {
        this.props.database.markAllAsUnpacked()
                           .then(this.fetchItems)
                           .catch(console.error);            
    }

    deleteUnpackedItems() {
        this.props.database.deleteUnpackedItems()
                           .then(this.fetchItems)
                           .catch(console.error);
    }
